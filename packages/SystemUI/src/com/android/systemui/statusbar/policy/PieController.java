...
public class PieController implements BaseStatusBar.NavigationBarCallback, PieView.OnExitListener,
    ...
    private void getCustomActionsAndConstruct(ContentResolver resolver,
        ...
        if (!secondLayer) {
            mMenuButton = constructItem(1, SlimActionConstants.ACTION_MENU,
    ...
    private void addItemToLayer(PieSliceContainer layer, ActionConfig ActionConfig,
        ...
        if (ActionConfig.getClickAction().equals(SlimActionConstants.ACTION_HOME)) {
            layer.addItem(constructItem(buttonWidth,
                    SlimActionConstants.ACTION_KEYGUARD_SEARCH,
                    ActionConfig.getLongpressAction(),
                    SlimActionConstants.ICON_EMPTY,
                    minimumImageSize));
        }
    ...
    private PieItem constructItem(int width, String clickAction, String longPressAction,
        ...
        if (!longPressAction.equals(SlimActionConstants.ACTION_NULL)) {
    ...
    private int setPieItemIcon(ImageView view, String iconUri, String clickAction) {
        ...
        if (iconUri != null && !iconUri.equals(SlimActionConstants.ICON_EMPTY)
            && !iconUri.startsWith(SlimActionConstants.SYSTEM_ICON_IDENTIFIER)) {
            if (clickAction.equals(SlimActionConstants.ACTION_BACK)) {
    ...
    protected void setNavigationIconHints(int hints, boolean force) {
        ...
        for (int j = 0; j < 2; j++) {
            item = findItem(SlimActionConstants.ACTION_BACK, j);
            if (item != null) {
    ...
    protected void setDisabledFlags(int disabledFlags, boolean force) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            item = findItem(SlimActionConstants.ACTION_HOME, j);
            if (item != null) {
                item.show(!disableHome);
                // If the homebutton exists we can assume that the keyguard
                // search button exists as well.
                item = findItem(SlimActionConstants.ACTION_KEYGUARD_SEARCH, j);
                item.show(disableHome);
            }
            item = findItem(SlimActionConstants.ACTION_RECENTS, j);
            if (item != null) {
    ...
    public void onClick(PieItem item) {
        ...
        if (!type.equals(SlimActionConstants.ACTION_MENU)) {
